...
def get_video_urls(channel_url):
    ...
    ydl_opts = {
        ...
        'browser_cookie_dirs': ['mnt/cookies/manifests'],
                    ...
                    if os.path.exists(filename):
                        ...
                        if bucket:
                            try:
                                ...
                                relative_path = os.path.relpath(filename, download_path)
                                
                                if upload_audio_to_gcs(bucket, filename, relative_path):
                                    logger.info(f"☁️ Uploaded to GCS: {filename}")
                                    # Delete local file after successful upload
                                    os.remove(filename)
                                    logger.info(f"🗑️ Deleted local file: {filename}")
                                    return True, True  # download success, upload success
                                else:
                                    logger.warning(f"⚠️ Failed to upload to GCS: {filename}")
                                    return True, False  # download success, upload failed
                            except Exception as e:
                                logger.error(f"❌ Error during GCS upload: {e}")
                                return True, False  # download success, upload failed
                        else:
                            logger.warning(f"⚠️ No GCS bucket available, keeping local file: {filename}")
